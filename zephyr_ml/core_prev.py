...
class Zephyr:
    ...
    def __eq__(self, other):
        return (
            ...
            and self._pipeline == other._pipeline
            and self._hyperparameters == other._hyperparameters
            and self._fitted == other._fitted
        )
    ...
    def fit_predict(
        self, X: pd.DataFrame, y: Union[pd.Series, np.ndarray], **kwargs
    ) -> pd.Series:
    ...
    def evaluate(
        ...
        train_y: Union[pd.Series, np.ndarray] = None,
        ...
        scores = {metric: METRICS[metric](y, result) for metric in metrics}
